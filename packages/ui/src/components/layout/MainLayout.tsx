...
export const MainLayout: React.FC = React.memo(() => {
  ...
  const handleRequestPushPR = useCallback(async () => {
    ...
    const pushPrompt = [
      ...
      '   - If the selected remote is not "origin", set gh --head to <origin-owner>:<branch-name>',
      '     - <origin-owner> must be parsed from the origin URL',
      '     - <branch-name> is the current branch name',
      '',
