...
import { ToolPanel, CreatePanelRequest, ToolPanelState, ToolPanelMetadata, ToolPanelType, LogsPanelState } from '@snowtree/core/types/panels';
...
export class PanelManager {
  ...
  constructor() {
    // Don't load panels in constructor - wait for initialize() to be called
  }

  /**
   * Initialize the panel manager by loading panels from database
  ...
  async deletePanel(panelId: string): Promise<void> {
    return await withLock(`panel-delete-${panelId}`, async () => {
      ...
      // No event subscriptions to clean up in minimal mode.
